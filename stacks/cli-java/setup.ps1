...
if (Test-Command "javac") {
    Write-Host "[OK] JDK already installed: $(javac -version 2>&1)" -ForegroundColor Green
} elseif ($hasWinget) {
    if (Install-Winget "EclipseAdoptium.Temurin.21.JDK" "Temurin JDK 21") {
        Refresh-Path
        if (Test-Command "javac") {
            Write-Host "[OK] JDK installed: $(javac -version 2>&1)" -ForegroundColor Green
        } else {
            Write-Host "[FAIL] JDK installed but not on PATH. Close and reopen PowerShell, then re-run." -ForegroundColor Red
            exit 1
        }
    } else {
        Write-Host "[FAIL] JDK install failed via winget" -ForegroundColor Red
        exit 1
    }
} else {
    Write-Host "[FAIL] JDK missing and winget unavailable. Install from https://adoptium.net/" -ForegroundColor Red
    exit 1
}
...
$missing = @()
foreach ($tool in @('git','node','npm','claude','uv','javac','just','gh')) {
...
Write-Host "  1. Compile the app:                   just build" -ForegroundColor Gray
Write-Host "  2. Run it with the sample input:      just run" -ForegroundColor Gray
Write-Host "  3. Run it with your own typed input:  just run-interactive" -ForegroundColor Gray
