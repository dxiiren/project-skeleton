...
# ---------- 2. Claude Code CLI ----------
# This repo has no package.json, so Node.js is not installed by this script. The
# Claude Code CLI ships via npm -- if npm is absent (fresh PC), we warn and move on
# rather than pulling in a whole Node toolchain the app itself never uses.
Refresh-Path
if (Test-Command "claude") {
    ...
    Write-Host "[WARN] npm not found -- Claude Code CLI skipped. Install Node.js LTS" -ForegroundColor Yellow
    Write-Host "       (winget install OpenJS.NodeJS.LTS) then re-run this script." -ForegroundColor DarkGray
}
...
# The app itself is plain PHP with session state only -- it needs no extra
# extensions. The fresh PHP zip has no php.ini (only ini-development /
# ini-production templates), so we copy the development template and enable a
# few bundled extensions so common PHP tooling works out of the box.
if (Test-Path $phpExe) {
...
"@
        ...
        Write-Host "[INFO] Wrote php.ini from the development template" -ForegroundColor Cyan
    }
    Add-UserPath $phpDir
...
Write-Host "  2. Start the dev server on http://127.0.0.1:@@PORT@@ :" -ForegroundColor Gray
