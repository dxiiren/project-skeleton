...
# ---------- 6. MSBuild (.NET Framework build toolchain) ----------
# Detection only -- NEVER auto-install VS Build Tools here: its installer needs UAC
# elevation and hangs unattended runs. Framework MSBuild ships with Windows and builds
# this solution fine (with three benign warnings -- see .docs/06-troubleshooting).
$msbuildBT = "${env:ProgramFiles(x86)}\Microsoft Visual Studio\2022\BuildTools\MSBuild\Current\Bin\MSBuild.exe"
$msbuildFW = "$env:WINDIR\Microsoft.NET\Framework64\v4.0.30319\MSBuild.exe"
if (Test-Path $msbuildBT) {
    Write-Host "[OK] MSBuild (VS Build Tools 2022): $msbuildBT" -ForegroundColor Green
} elseif (Test-Path $msbuildFW) {
    Write-Host "[OK] MSBuild (.NET Framework, ships with Windows): $msbuildFW" -ForegroundColor Green
    Write-Host "     Builds this solution with 3 benign warnings (ToolsVersion fallback, MSB3644, MSB3270)." -ForegroundColor DarkGray
    Write-Host "     Optional: installing VS Build Tools 2022 (manual, elevated -- 'winget install Microsoft.VisualStudio.2022.BuildTools' from an admin shell) removes them." -ForegroundColor DarkGray
} else {
    Write-Host "[FAIL] No MSBuild found. Expected at least Framework MSBuild at $msbuildFW" -ForegroundColor Red
    exit 1
}
...
if ((Test-Path $msbuildBT) -or (Test-Path $msbuildFW)) {
...
Write-Host "  1. Build the solution:      just build" -ForegroundColor Gray
Write-Host "  2. Launch the app (GUI):    just run    (close it with: just stop)" -ForegroundColor Gray
